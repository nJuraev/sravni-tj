...
| `key_conditions_ru` | array<string> | да | ключевые условия продукта буллетами (рус.); `null` — на странице банка нет такого текста |
| `key_conditions_tg` | array<string> | да | ключевые условия продукта буллетами (тадж.) |
| `documents_ru` | array<string> | да | минимальный пакет документов (рус.) |
| `documents_tg` | array<string> | да | минимальный пакет документов (тадж.) |
| `source_url` | string | да | ссылка на страницу именно этого продукта на сайте банка |
...
Публичная страница банка (карточка: телефон, email, адрес, короткая справка `about_*`).
404 для несуществующего/неактивного банка. Схема ответа — объект `Bank` (см. ниже), `data` — не массив.
...
| `address_tg` | string | да | адрес (тадж.) |
| `about_ru` | string | да | короткая справка о банке для публичной страницы `/bank/:id` (рус.) |
| `about_tg` | string | да | короткая справка о банке (тадж.) |
| `contact_email` | string | да | справочный email (НЕ адрес доставки заявок) |
| `rating_avg` | number | да | средний балл по одобренным отзывам |
| `rating_count` | integer | нет | число одобренных отзывов |

> Доступна также по `GET /api/banks/{id}` (публичная страница банка) — та же схема, один объект в `data`.
> Поля `email` (адрес доставки заявок) и `status` НЕ отдаются наружу (внутренние/служебные).
