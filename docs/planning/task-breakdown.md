...
4. Стартовый статус распарсенного продукта — **`active`** (парсер вставляет явно, без ручного подтверждения). БД `DEFAULT` колонки — `draft` (используется прочими путями вставки, не парсером).
