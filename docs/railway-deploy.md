...
| **parser** | `parser` | Nixpacks (cron) | Go-парсер, по расписанию |
...
Root Directory: `parser`. Nixpacks ([railway.json](../parser/railway.json)): `go build -o app ./cmd/parser`, запуск `./app` по расписанию `0 3 * * *` (ежедневно 03:00 UTC), `restartPolicyType: NEVER` (разовый прогон).
