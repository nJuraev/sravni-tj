...
Root Directory: `parser`. Сборка по [Dockerfile.railway](../parser/Dockerfile.railway) (отдельный от dev-`Dockerfile`, который используется только в docker-compose): multi-stage, `go build -o app ./cmd/parser`, запуск `./app` по расписанию `0 3 * * *` (ежедневно 03:00 UTC), `restartPolicyType: NEVER` (разовый прогон).
