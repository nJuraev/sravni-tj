...
func productItemSchema() map[string]any {
	...
	product := map[string]any{
		...
		"properties": map[string]any{
			...
			"key_conditions_ru": stringListSchema("Ключевые условия продукта буллетами на русском (кроме ставки/суммы/срока — они уже в rate_tiers, даже если на странице это текстовый буллет вида «Ставка от N до M дней/месяцев: X%», а не таблица): предоплата, комиссия по сегментам клиентов, залог/обеспечение и т.п. null/[] — на странице нет такого текста."),
			...
			"rate_tiers": map[string]any{
				...
				"minItems":    0,
				"maxItems":    50,
				"description": "Тарифная сетка: ставка зависит от срока И/ИЛИ суммы. Разбирай сюда и текстовые буллеты вида «Ставка от N до M дней/месяцев: X%» (дни → месяцы, 30 дней = месяц), не только табличные сетки. Единая ставка → один элемент. Не выдумывай диапазоны.",
				"items":       tierItem,
			},
